...
socket.on('message', function(message, room) {
    ...
    if ((typeof(message) == "string" &&  message.includes("bye"))) {
      simpleToast(" ⚠️ "+OtherPeerUsername + " left","#f39c12");
    }
});
...
socket.on('ExchangePublicKeyNow',({ PublicKey, room ,username}) =>{
  ...
  simpleToast(" ℹ️ "+username + " Joined","#28a745");
    ...
    socket.on('createMessage', ({username,message}) => {
      ...
      var messageToShow = (this.username == username) ? LocalMessage : decodedMessage;
        ...
        <div class="direct-chat-infos clearfix">
          <span class="direct-chat-name float-${ChatUserNameClass}">${username}</span>
          <span class="direct-chat-timestamp float-${ChatTimeClass}">${hours}:${minutes}</span>
        </div>
...
//<li class="message"><b>user</b><br />${username} : ${message}</li>
 ...
 function simpleToast(msg,msgColor) {
  ...
  $("#simpleToast").css("background-color", msgColor);
